...
| `ws.disable-methods`
| A list of RPC methods that should _not_ be sent via WebSocket connection, even if it's available. The HTTP JSON RPC will be used instead. Ex. large `trace_` calls that overload the WebSocket connection. Default is empty list.
...
| `tls`
a| TLS configuration for the connection.
...
A `url` with the `https` scheme always connects over TLS, verifying the server against the system root certificates, even when this section is omitted.
A `host` + `port` connection without this section is made in plaintext. +
