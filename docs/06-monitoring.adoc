...
- _Egress Log_ - requests made to Dshackle from a client
- _Ingress Log_ - requests made by Dshackle to upstreams

Because some requests may produce multiple responses, or multiple internal requests the log is not related as one-to-one.
But instead maintains a reference a `request.id` in both logs, that can be used to join multiple egress and ingress logs into a single trace.

== Egress Log

NOTE: By default, the egress log is disabled.

To enable egress log add following configuration:

[source,yaml]
----
egress-log:
  enabled: true
  filename: /var/log/dshackle/egress_log.jsonl
----

`filename` is optional, and the default value is `egress_log.jsonl` (i.e., in the current directory).
...
NOTE: By default, the ingress log is disabled.

To enable ingress log add following configuration:

[source,yaml]
----
ingress-log:
  enabled: true
  filename: /var/log/dshackle/ingress_log.jsonl
----
...
** `id` for a `REQUEST` source it's the same id as in egress log in `request.id` field.
