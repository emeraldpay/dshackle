...
class QuorumRpcReaderTest : ShouldSpec({
    ...
    context("always quorum") {
        should("get the result if ok") {
            ...
            val act = reader.read(JsonRpcRequest("eth_test", emptyList()))
                .map { String(it.value) }
        ...
        should("return upstream error") {
            ...
            val t = shouldThrowAny {
                reader.read(JsonRpcRequest("eth_test", emptyList()))
                    .map { String(it.value) }
                    .block(Duration.ofSeconds(1))
        ...
        should("return upstream error thrown by reader") {
            ...
            every { upstream.getRole() } returns UpstreamsConfig.UpstreamRole.PRIMARY
            every { upstream.getIngressReader() } returns mockk() {
                every { read(JsonRpcRequest("eth_test", emptyList())) } returns Mono.error(RpcException(RpcResponseError.CODE_UPSTREAM_CONNECTION_ERROR, "test"))
            }
