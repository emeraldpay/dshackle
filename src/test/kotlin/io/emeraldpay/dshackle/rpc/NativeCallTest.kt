    ...
    ShouldSpec({
        ...
        context("Parse input") {
            should("prepare call as error for invalid blockchain") {
                ...
                val act =
                    shouldThrowAny {
                        native
                            .prepareCall(BlockchainOuterClass.NativeCallRequest.getDefaultInstance())
                            .collectList()
                            .block(Duration.ofSeconds(1))
        ...
        context("Response") {
            ...
            should("build success response with signature") {
                ...
                val act =
                    native.buildResponse(
                        NativeCall.CallResult.ok(
                            1051,
                            65432,
                            "true".toByteArray(),
                            ResponseSigner.Signature(Hex.decodeHex("010203"), "test1", 100),
                        ),
                    )
