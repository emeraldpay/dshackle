...
class GrpcUpstreamStatusTest : ShouldSpec({

    val baseConf = BlockchainOuterClass.DescribeChain.newBuilder()
        .setStatus(
    ...
    should("Not indicate update if nothing changed") {
        ...
        val state = GrpcUpstreamStatus().also {
    ...
    should("Indicate update if methods changed") {
        ...
        val updated = state.update(
            BlockchainOuterClass.DescribeChain.newBuilder(baseConf)
