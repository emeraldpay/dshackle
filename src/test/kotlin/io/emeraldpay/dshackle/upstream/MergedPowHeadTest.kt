...
class MergedPowHeadTest :
    ShouldSpec({
        ...
        should("Follow") {
            ...
            val act =
                head
                    .merge(listOf(Flux.just(blocks[0])))
        ...
        should("Follow normal order") {
            ...
            val act =
                head
                    .merge(listOf(Flux.just(blocks[0], blocks[1], blocks[3])))
                    .take(Duration.ofMillis(500))
        ...
        should("Ignore old") {
            ...
            val act =
                head
                    .merge(listOf(Flux.just(blocks[0], blocks[3], blocks[1])))
                    .take(Duration.ofMillis(500))
            StepVerifier
                .create(act)
                .expectNext(blocks[0])
        ...
        should("Ignore repeating") {
            ...
            val act =
                head
                    .merge(listOf(Flux.just(blocks[0], blocks[3], blocks[3], blocks[3], blocks[2], blocks[3])))
                    .take(Duration.ofMillis(500))
            StepVerifier
                .create(act)
                .expectNext(blocks[0])
        ...
        should("Ignore less difficult") {
            ...
            val act =
                head
                    .merge(listOf(Flux.just(blocks[0], blocks[3], block3less)))
                    .take(Duration.ofMillis(500))
            StepVerifier
                .create(act)
                .expectNext(blocks[0])
