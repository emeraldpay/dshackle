...
class EthereumPriorityFeesTest :
    ShouldSpec({
        ...
        should("Estimate") {
            ...
            val ups =
                mockk<EthereumMultistream> {
                    every { getHead() } returns
                        mockk<Head> {
            ...
            val reader =
                mockk<DataReaders> {
                    every { blocksByHeightParsed } returns
                        mockk<Reader<Long, BlockJson<TransactionRefJson>>> {
                            every { read(13756006) } returns Mono.just(block1)
                            every { read(13756007) } returns Mono.just(block2)
                        }
                    every { txReaderParsed } returns
                        mockk<Reader<TransactionId, TransactionJson>> {
                            every { read(TransactionId.from("0x22222222fad596cad644b785a8a74f6580ceec9ae13c8aa174f819c0223b8c77")) } returns
                                Mono.just(tx1)
                            every { read(TransactionId.from("0x55555555fad596cad644b785a8a74f6580ceec9ae13c8aa174f819c0223b8c77")) } returns
                                Mono.just(tx2)
                        }
                }
