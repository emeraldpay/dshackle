...
class DefaultUpstreamTest :
    ShouldSpec({
        ...
        should("Availability for always valid upstream doesn't react on old height") {
            val upstream =
                DefaultUpstreamTestImpl(
                    "test",
                    ForkWatch.Never(),
                    UpstreamsConfig.PartialOptions
                        .getDefaults()
                        .apply {
...
class DefaultUpstreamTestImpl(
    ...
    override fun getIngressReader(): StandardRpcReader = throw NotImplementedError()

    override fun getLabels(): Collection<UpstreamsConfig.Labels> = throw NotImplementedError()

    override fun getCapabilities(): Set<Capability> = throw NotImplementedError()

    override fun isGrpc(): Boolean = false

    override fun <T : Upstream> cast(selfType: Class<T>): T = throw NotImplementedError()
}
