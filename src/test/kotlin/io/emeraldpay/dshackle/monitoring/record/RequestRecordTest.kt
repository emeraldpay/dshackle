...
class RequestRecordTest : ShouldSpec({

    val objectMapper = Global.objectMapper

    context("BlockchainRequest") {
        val record = RequestRecord.BlockchainRequest(
            id = UUID.randomUUID(), jsonrpc = null, upstream = null, success = true,
            request = RequestRecord.RequestDetails(
                id = UUID.randomUUID(), source = RequestRecord.Source.INTERNAL,
