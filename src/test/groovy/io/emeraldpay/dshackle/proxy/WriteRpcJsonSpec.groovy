...
class WriteRpcJsonSpec extends Specification {
    ...
    def "Convert basic to JSON"() {
        ...
        call.ids[1] = 105
        def data = [
                new NativeCall.CallResult(1, null, '"0x98dbb1"'.bytes, null, null, null)
        ]
        when:
    ...
    def "Convert gRPC error to JSON"() {
        ...
        call.ids[1] = 1
        def data = [
                new NativeCall.CallResult(1, null, null, new NativeCall.CallError(1, "Internal Error", null), null, null)
        ]
        when:
        def act = writer.toJson(call, data[0])
        then:
    ...
    def "Convert few items to JSON"() {
        ...
        def data = [
                new NativeCall.CallResult(1, null, '"0x98dbb1"'.bytes, null, null, null),
                new NativeCall.CallResult(2, null, null, new NativeCall.CallError(2, "oops", null), null, null),
                new NativeCall.CallResult(3, null, '{"hash": "0x2484f459dc"}'.bytes, null, null, null),
        ]
        when:
    ...
    def "Write JSON RPC error on exception"() {
        ...
        call.ids[1] = 10
        def data = [
                new NativeCall.CallResult(1, null, '"0x1"'.bytes, null, null, null),
        ]
        when:
        def act = Flux.fromIterable(data)
                .transform(writer.toJsons(call))
