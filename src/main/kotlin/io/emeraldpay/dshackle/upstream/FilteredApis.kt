...
class FilteredApis(
    ...
    companion object {
        ...
        private const val METRICS_CODE = "select"
    ...
    override fun subscribe(subscriber: Subscriber<in Upstream>) {
        ...
        val retries =
            (0 until (retryLimit - 1))
                .map { r ->
                    Flux
                        .fromIterable(standardWithFallback)
        ...
        result
            .filter { up -> !excluded.contains(up) && up.isAvailable() && matcher.matches(up) }
    ...
    override fun toString(): String = "Filter API: ${allUpstreams.size} upstreams with $matcher"

    class Monitoring(
        chain: Chain,
    ) {
        val countPrimary: DistributionSummary =
            DistributionSummary
                .builder("$METRICS_CODE.exist")
